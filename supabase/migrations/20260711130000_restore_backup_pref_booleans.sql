-- Extend restore_backup's user_preferences upsert to carry the two boolean
-- visibility prefs (roadmap slices S-14 show_fire_dashboard, S-18 show_drift_alerts).
--
-- Gap being closed: the original restore_backup RPC
-- (20260620120000_restore_backup_rpc.sql) whitelists user_preferences columns
-- explicitly and never listed show_fire_dashboard — so backup IMPORT silently
-- failed to restore it (export already carries it via the backup.ts whitelist).
-- show_drift_alerts (added in 20260711120000) inherits the same gap. This
-- recreates the function with BOTH booleans in the INSERT column list and the
-- ON CONFLICT DO UPDATE SET, so the toggle state round-trips through import.
--
-- Both columns are NOT NULL DEFAULT TRUE, so a lower-version backup that omits
-- them COALESCEs to TRUE rather than violating NOT NULL — matching the
-- re-default pattern the other NOT-NULL-with-default columns already use.
--
