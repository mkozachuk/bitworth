...
- **Outcome:** on the assets page the user can enter an explicit "Edit list" mode, which reveals a drag handle on every row, and drag assets into the order they want — the order they think about their portfolio in, rather than the reverse-chronological order they happened to enter them in. The chosen order persists across sessions and devices, works on both the desktop table and the mobile card list (S-07), and is reachable by keyboard. Leaving edit mode returns the list to its normal read/act state (Edit / Delete untouched).
- **Change ID:** `asset-list-reorder`
- **PRD refs:** — (post-MVP assets-page UX extension; not a PRD functional requirement)
- **Prerequisites:** `F-01` (schema — new ordering column on `assets`), `S-01` (asset CRUD + the list this extends), `S-07` (the two list renderings a drag surface must cover: desktop `<table>`/`AssetRow` and mobile `<ul>`/`AssetCard`)
- **Parallel with:** all slices after `S-07` (self-contained: one column + one write endpoint + one list interaction; no other slice reads asset order)
- **Blockers:** —
- **Unknowns:**
  - **Persistence model & the write path (the core unknown).** `assets` has no ordering column today; both read paths sort by `created_at DESC` (`src/pages/dashboard/assets/index.astro:25` and `src/pages/api/assets/index.ts:38`). Options: (a) an integer `sort_order` rewritten for the whole list on each drop; (b) fractional/`NUMERIC` positions so a single drop updates one row; (c) a separate ordering table. (Owner: planner, by: during `/10x-plan`) Recommendation: **(a)** — `sort_order INTEGER NOT NULL DEFAULT 0` via a column-add migration, backfilled from the current `created_at DESC` order so nothing shuffles on deploy. Manual-entry portfolios are small (tens of rows), so a full rewrite is cheap and avoids the precision drift fractional indexing eventually hits. Both read paths must switch to `.order("sort_order").order("created_at", { ascending: false })` — the `created_at` tiebreak keeps the ordering total if two rows ever share a `sort_order`.
  - **Atomicity of the bulk write.** Reordering is a multi-row update with no Supabase JS transaction — a partial write leaves a list whose order is neither the old one nor the new one. This is the same trap the S-02 lesson recorded for multi-table writes. (Owner: planner) Recommendation: a single Postgres RPC (e.g. `reorder_assets(p_ids uuid[])`) that renumbers in one statement and is `SECURITY DEFINER`-scoped to `auth.uid()` (so a foreign asset id in the array is a no-op, not a cross-user write); mirror the `restore_backup` RPC + grants precedent (`20260620120000` / `20260621000000`). Endpoint: `PATCH /api/assets/order` taking the full ordered id array.
  - **Backup completeness.** A new `assets` column is silently dropped on import unless it is added to **both** the `ASSET_FIELDS` whitelist in `src/lib/backup.ts:55-70` **and** the `restore_backup` assets INSERT/SELECT lists via a new migration. (Owner: planner) Recommendation: land it in the same phase as the column, following the `metal_symbol` precedent exactly (`20260712130000_restore_backup_metal_symbol.sql`) — that migration exists *because* this step was missed once. `sort_order` is `NOT NULL`, so the restore needs a `COALESCE(r.sort_order, 0)` like `show_on_chart` has.
  - **Drag mechanics & library.** No drag library is in the dependency set today. Options: `@dnd-kit/core` + `@dnd-kit/sortable`, `react-dnd`, native HTML5 drag-and-drop (no touch support — disqualifying, since S-07/S-08 make mobile a first-class surface), or hand-rolled pointer events. (Owner: planner, by: during `/10x-plan`) Recommendation: **`@dnd-kit`** — pointer + touch + keyboard sensors out of the box, with `SortableContext` handling both a `<ul>` and a `<table>` body. Verify React 19 compatibility before committing (the S-03/S-19 "verify before you commit to a provider" lesson, applied to a dependency). Two mechanics to get right: a `touch-action: none` handle plus an activation constraint (small delay/tolerance) so a vertical swipe still scrolls the page on iOS Safari rather than starting a drag; and `transform` on `<tr>` elements, which needs the table variant of dnd-kit's styling.
  - **Interaction with the filter tabs.** `AssetList` filters to All / Assets / Liabilities (`src/components/assets/AssetList.tsx:31-35`); dragging inside a filtered subset means reordering rows whose neighbours are hidden. (Owner: planner) Recommendation: allow edit mode only when `filter === "all"` (disable the toggle with a hint on the other tabs) — the alternative, permuting only the visible rows' `sort_order` values, is defensible but hard to explain and easy to get subtly wrong.
  - **Save timing.** Persist on each drop vs batch and save on "Done". (Owner: planner) Recommendation: optimistic save on drop (local state reorders immediately, `PATCH` fires, revert + error message on failure) so a closed tab never loses work; the edit-mode toggle then governs only handle visibility and hit-target layout, not persistence. Reuse the existing inline `deleteError` pattern in `AssetList` for the failure surface rather than adding a new toast system.
...
| S-25       | asset-list-reorder         | Drag-and-drop asset ordering in an "edit list" mode              | yes                   | depends on F-01/S-01/S-07; new `assets.sort_order INTEGER NOT NULL DEFAULT 0` column backfilled from `created_at DESC` + both read paths re-sorted (`dashboard/assets/index.astro`, `api/assets/index.ts`); atomic `reorder_assets(uuid[])` RPC behind `PATCH /api/assets/order` (no JS transactions — S-02 lesson); **thread the column into `backup.ts` ASSET_FIELDS *and* the `restore_backup` RPC in the same phase (`metal_symbol` precedent) or restores silently drop the order**; `@dnd-kit` (verify React 19 compat) across both the desktop `<table>` and mobile `<ul>` renderings, keyboard sensor + aria-live; edit mode gated to the "All" filter tab |
